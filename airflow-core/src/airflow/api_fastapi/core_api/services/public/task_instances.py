    ...
    for ti in updated_tis:
        try:
            if new_state == TaskInstanceState.SUCCESS:
                ...
                # A user marking a task failed is a user-sourced action, not an
                # infra disruption — so it is never refunded or resumed (both gate on
                # failure_kind == "infra"). A plain "user" kind states that without
                # overloading any infra-named field.
