...
@pytest.mark.parametrize(
    ...
    [
        # user-initiated stops — never refunded, whatever signal did the killing
        ("mark_task_failed", TaskFailureKind.USER, False),
        ("mark_dagrun_failed", TaskFailureKind.USER, False),
        ("app_exception", TaskFailureKind.USER, False),
        ("execution_timeout", TaskFailureKind.TIMEOUT, False),
        ("unclassified", None, False),
        # infrastructure disruption — the only refunded source
        ("pod_evicted", TaskFailureKind.INFRA, True),
        ("worker_death_zombie", TaskFailureKind.INFRA, True),
    ],
)
@conf_vars(ENABLED)
