Add the ``TaskFailureInfo`` value object and extend the ``on_task_instance_failed`` listener hook to accept it as a ``failure_details`` keyword argument (AIP-97 foundation).
...
``TaskFailureInfo`` is the executor-agnostic shape every executor populates
