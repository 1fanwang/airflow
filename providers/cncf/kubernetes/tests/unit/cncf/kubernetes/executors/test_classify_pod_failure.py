...
class TestClassifyPodFailure:
    ...
    def test_end_to_end_oomkilled_pod_is_user(self):
        ...
        # collect_pod_failure_details -> classify_pod_failure and classifies as user, not infra,
    ...
    def test_end_to_end_evicted_pod_is_infra(self):
        ...
        pod = k8s.V1Pod(
            ...
            status=k8s.V1PodStatus(phase="Failed", reason="Evicted", message="The node was low on resource: memory."),
        )
        details = collect_pod_failure_details(pod, logging.getLogger("test"))
        assert details is not None
