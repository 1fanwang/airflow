        ...
        *,
        ...
        if state == TaskInstanceState.FAILED:
            ...
            if failure_details:
                ...
                # AIP-97: classify this pod failure (Evicted / OOMKilled / node
                # shutdown / ...) into a TaskFailureInfo. The scheduler reads it via
                # get_task_failure_info() and hands it to handle_failure, so an infra
                # disruption is told from the task's own failure and the retry budget
                # is charged accordingly. An app OOM against its own limit stays "user".
                from airflow.providers.cncf.kubernetes.executors.kubernetes_executor_utils import (
                    to_task_failure_info,
                )

                task_failure_info = to_task_failure_info(failure_details)
                if task_failure_info is not None:
                    self.task_failure_info[key] = task_failure_info
            else:
                task_key_str = f"{key.dag_id}.{key.task_id}.{key.try_number}"
                self.log.warning(
