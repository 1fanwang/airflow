...
class TestEcsFailureClassification:
    ...
    @pytest.mark.db_test
    @pytest.mark.parametrize(
        ("cap", "stop_code", "try_number", "max_tries", "expected_state", "expected_metric"),
        [
            pytest.param(
                1,
                "SpotInterruption",
    ...
    def test_ecs_failure_uses_deployment_infra_budget(
        self,
        mock_executor,
        dag_maker,
        session,
        ...
        with dag_maker(dag_id="ecs_failure_budget", fileloc="/ecs/"):
            task = EmptyOperator(task_id="task", retries=0)
        ti = dag_maker.create_dagrun(state=DagRunState.RUNNING).get_task_instance(
            task.task_id,
            session=session,
        )
        ti.state = State.QUEUED if stop_code == "TaskFailedToStart" else State.RUNNING
        ...
        mock_executor.max_run_task_attempts = "1"
        self._add_mock_task(mock_executor, ARN1, task_key=ti.key)
        mock_executor.ecs.describe_tasks.return_value = {
            "tasks": [self._stopped_task_json(stop_code, "ECS task stopped")],
            "failures": [],
        }
        ...
        with (
            ...
            mock.patch("airflow.models.taskinstance.stats.incr", autospec=True) as incr,
        ):
        ...
        granted = expected_metric == "ti_infra_retry_granted"
        assert (ti.state, ti.max_tries, ti.try_number) == (expected_state, max_tries + granted, try_number)
        policy_metrics = [
            call.args[0]
            for call in incr.call_args_list
            if call.args[0] in {"ti_infra_retry_granted", "ti_infra_retry_denied"}
        ]
        assert policy_metrics == ([expected_metric] if expected_metric else [])
